...
async def send_realtime_hit_dm(user_id, result, hit_type, first_name):
    try:
        ...
        dm_msg = f"""<b>💎 HIT DETECTED 💎</b>
━━━━━━━━━━━━━━━━━━━━
<b>⚡️ Status ➜ {status_text}</b>
<b>⭐ Resp ➜ {response_msg}</b>
━━━━━━━━━━━━━━━━━━━━
<b>💵 Amount ➜ {currency}{price}</b>
<b>💳 CC ➜ <tg-spoiler><code>{result['card']}</code></tg-spoiler></b>
<b>💳 Bin ➜ {result['card'][:6]} - {brand}</b>
<b>🏧 Bank ➜ {bank}</b>
<b>☄️ Country ➜ {country} {flag}</b>
<b>⏳ Time ➜ {current_time}</b>
<b>👑 By ➜ <a href="tg://user?id={user_id}">{first_name}</a> [{plan}]</b>
<b>🦄 Bot By: <a href="tg://user?id={ADMIN_ID}">{OWNER_NAME}</a></b>"""
        buttons = [[Button.inline("📋 COPY CC", f"copycc_{result['card']}".encode())]]
...
<b>👥 Group:</b> <a href="{GROUP_LINK}">HitByDark</a>"""
    ...
    if is_admin(user_id):
        buttons.append([Button.inline("🛠 ADMIN PANEL", b"admin_panel")])
    await event.edit(premium_emoji(welcome), buttons=buttons, parse_mode="html")
